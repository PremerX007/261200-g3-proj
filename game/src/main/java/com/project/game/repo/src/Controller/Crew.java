...
public class Crew {
    ...
    protected void goBackCityCenter(){
        Territory.instance.setCrewStatus(new Position(row,col), new Position(citycenter.i, citycenter.j));
    ...
    protected void move(int direction){
        ...
        if(Territory.instance.checkCrewCanMove(president,target)){
            if(president.getBudget() >= 1){
                Territory.instance.setCrewStatus(new Position(row,col), target);
                this.row = target.i;
                this.col = target.j;
                president.payCost(1);
            } else {
