...
public class PlanTokenizer implements Tokenizer{
    ...
    public void checkNextToken() {
        if (!hasNextToken()) throw new NoSuchElementException("no more tokens, please check your construction plan");
    }
    ...
    public void computeNext() throws LexicalError {
        ...
        if (Character.isDigit(c)) { // start of number
            ...
            s.append(c);
            for (pos++; pos < src.length() && (Character.isLetter(src.charAt(pos)) || src.charAt(pos) == '_' || Character.isDigit(src.charAt(pos))); pos++){
                s.append(src.charAt(pos));
            }
        }
